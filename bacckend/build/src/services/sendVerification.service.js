...
exports.VerifyEmailSend = void 0;
const nodemailer_1 = __importDefault(require("nodemailer"));
const user_service_1 = require("./user.service");
const VerifyEmailSend = async (email) => {
    const user = await (0, user_service_1.FindUserEmail)(email);
    if (!user) {
...
`,
    ...
    return new Promise((resolve, reject) => {
        transport.sendMail(mailOptions, (error, info) => {
            if (error) {
                reject(new Error("Erro ao enviar e-mail de verificação: " + error.message));
            }
            else {
                resolve({
