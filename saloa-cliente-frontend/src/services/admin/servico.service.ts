...
export const GetAllServicesById = async (id: string): Promise<service> => {
  const response = await apiAdmin.get(`/services/${id}`, {});
...
export const PostAgendamento = async (data: { serviceId: string, professionalId: string, date: string }) => {
  try {
    const res = await apiAdmin.post<agendado>("/appointments", data)
    console.log(res)
    return res.data
  } catch (error) {
    console.log(error)
...
export const DeleteAgendamentoById = async (id: string): Promise<boolean | undefined> => {
  try {
    const res = await apiAdmin.delete(`/appointments/${id}`)
    if (res.status === 200) {
      return true
    } else {
      return false
...
export const CancelAgendamentoById = async (id: string, status: "CONFIRMED" | "PENDING" | "CANCELLED" | 'COMPLETED'): Promise<boolean | undefined> => {
  try {
    const res = await apiAdmin.put(`/appointments/status/${id}`, {
      status
    })
    if (res.status === 200) {
      return true
